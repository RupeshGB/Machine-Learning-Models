...
class SVDRecommender(object):
    ...
    def recommender_svd(self, R: np.ndarray, k: int) ->Tuple[np.ndarray, np
        ...
        U_k = U[:, :k]
        VT_k = VT[:k, :]  # Top k rows from V^T

        # If V_k needs to have a shape of (M, k), you would transpose VT_k
        V_k = VT_k.T  # Now V_k has a shape of (M, k)


        # Return the matrices U_k and V_k (transpose of VT_k to get V_k)
        return U_k, VT_k.T
    ...
    def predict(self, R: np.ndarray, U_k: np.ndarray, V_k: np.ndarray,
        ...
        watched_movies_idx = np.where(R[user_idx, :] > 0)[0]  # Movies already watched (rated) by the user

        # Predict ratings for all movies in movies_pool that haven't been watched yet
        movie_predictions = []
        for movie_name in movies_pool:
            movie_id = self.get_movie_id_by_name(movie_name)
            movie_idx = movies_index[movie_id]
            # Skip if the movie has already been watched
            if movie_idx in watched_movies_idx:
                continue
            # Predict rating using dot product of user and movie features
            predicted_rating = np.dot(U_k[user_idx, :], V_k[:, movie_idx])
            movie_predictions.append((movie_name, predicted_rating))

        # Sort movie predictions by predicted rating in descending order and select top_n
        movie_predictions.sort(key=lambda x: x[1], reverse=True)
        top_movies = [movie[0] for movie in movie_predictions[:top_n]]
